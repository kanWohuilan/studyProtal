<template>
  <n-alert title="" type="info" class="mb-6">
    演示账号和密码为：
    <n-tag :bordered="false" type="success"> admin/admin123 </n-tag>
  </n-alert>
...
<script setup>
import { NForm, NInput, NFormItem, NButton, NAlert, NTag, createDiscreteApi } from "naive-ui";
...
const onSubmit = () => {
  formRef.value.validate(async (errors) => {
    ...
    let { data } =
      type.value === "login"
        ? await useLoginApi(form).catch(({ error }) => {
